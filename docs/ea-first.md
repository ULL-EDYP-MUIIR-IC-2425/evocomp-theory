...
This is a first version where capacity constraint is not considered to repair
...
In the next version, individuals are repaired for the next generation:
...
def genetic_algorithm():
    ...
    for i in range(num_generations):
        ...
        children = crossover(parents)
        population = [mutate(child) for child in children]
        # enforce the knapsack constraint
